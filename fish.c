...
int main()
{
    ...
    for (;;) { // Assign the handler function to the sa struct
        ...
        else if (li.n_cmds == 1 && li.cmds[0].n_args > 1)
        {
            ...
            if (DEBUG)
                printf("%sSimple command WITH arguments!%s\n", GREEN, NC);
            exeSimpleCommand(li);
        }
...
int detectInternCommand(struct cmd cmd)
{
    ...
    if (DEBUG)
        printf("LEN == %d\n", len);
    if (len != 2 && len != 4)
        return -1; // because len("cd") == 2 and len("exit") == 4
...
int cd(struct cmd command)
{
    ...
    { // Check if there is too many args
...
void exeSimpleCommand(struct line li)
{
    ...
    else
    {
        ...
        if(!li.background) foreground_pid = pid; // We store the pid of the foreground process, to kill with Ctrl+C
        int status;
...
void sigint_handler(int signum) {
    printf("%sCaught SIGINT signal code : %d%s\n", GREEN, signum, NC);
    ...
    if (foreground_pid > 0) {
        ...
        if (DEBUG)
            printf("%sNo foreground process to kill%s\n", RED, NC);
    }
    line_reset(&li);
    printCommandLine(li);
}
